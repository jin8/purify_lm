...
ALLOWED_MODELS = ['gpt3', 'gpt2', 'dexperts', 'dexperts-gpt3', 'pplm', 'mapping','mapping_attention','att','attadd','cont_gpt', 'add_cont_bert_gpt','cont_bert_gpt', 'cont_bert_gpt_only','add_cont_gpt_last','cont_gpt_last', 'cont_gpt_last_only']
...
@click.option('--past-key-values/--no-past-key-values', default=False, type=bool, help='use past key value')
@click.option('--end-token-prediction/--no-end-token-prediction', default=False, type=bool, help='use end token prediction during the generation')
@click.option('--logit/--no-logit', default=False, type=bool, help='using logit subtraction')
@click.option('--softmax-logit/--no-softmax-logit', default=False, type=bool, help='using softmax to calculate logit')
@click.option('--logit_type', default=0, type=int, help='logit calculation formula type')
@click.option('--mixup-loss/--no-mixup-loss', default=False, type=bool, help='mixup')
def main(config_name:Optional[str], local_rank:int, num_examples:int, random_state:int, output_dir: str, dataset_file: Optional[str],
         ...
         alpha: float, filter_p: float, p: float, perspective_api: bool, past_key_values: bool, end_token_prediction: bool, logit: bool, softmax_logit: bool, logit_type: int, mixup_loss: bool):
    ...
    if dataset_file:
        ...
        dataset = None
        # Create EOS prompts
        if model_type in ['gpt2', 'gpt2-affect', 'gpt2-ensemble', 'gpt2-naughty-list', 'pplm','mapping','mapping_attention','att','attadd', 'add_cont_bert_gpt','cont_bert_gpt','cont_bert_gpt_only', 'cont_gpt', 'add_cont_gpt_last','cont_gpt_last', 'cont_gpt_last_only']:
    ...
    if model_type == 'gpt2':
        ...
        generations_iter = mapping(
            ...
            end_token_prediction=end_token_prediction,
            logit=logit,
            alpha=alpha,
