...
class AttGPT2(GPT2LMHeadModel):
    ...
    def mse_loss(self, t1, t2, n1, n2):
        
        total_mse, mse_distance=10,0
        
        mse_loss = nn.MSELoss()
        cnt = 0

        if t1.size(0)>0 and n1.size(0)>0:
            for nb in range((n1.size(0))):
                for tb in range((t1.size(0))):
                    #print(mse_loss(aft_n1[nb].view(-1), aft_t1[tb].view(-1)))
                    total_mse -= self.alpha_mse * mse_loss(n1[nb], t1[tb])
                    cnt += 1
        
        if not (t2 is None):
            if t2.size(0)>0 and n2.size(0)>0:
                for nb in range((n2.size(0))):
                    for tb in range((t2.size(0))):
                        #print(mse_loss(aft_n1[nb].view(-1), aft_t1[tb].view(-1)))
                        total_mse -= self.alpha_mse * mse_loss(n2[nb], t2[tb])
                        cnt += 1
        ...
        if labels is not None:
            ...
            if 'cont' in self.ablation:
                ...
                if dist.is_initialized() and self.training:
                    z1, z2, gather_attr_labels = self.gather(z1, z2, batch_attr_label)
                cos_loss = self.self_contrastive_loss(z1, z2, diff=False)
                if 'mse' in self.ablation:
                    toxic_index, non_index = (gather_attr_labels.squeeze() == 1), (gather_attr_labels.squeeze() == 0)
                    toxic_z1, non_z1 = z1[toxic_index], z1[non_index]
                    toxic_z2, non_z2 = z2[toxic_index], z2[non_index]
                    # prev_t1, prev_t2, aft_t1, aft_t2, prev_n1, prev_n2, aft_n1, aft_n2
            else:
                if 'mse' in self.ablation:
                    toxic_index, non_index = (attr_labels.squeeze() == 1), (attr_labels.squeeze() == 0)
                    z1 = mlp_output[:,0]
                    toxic_z1, non_z1 = z1[toxic_index], z1[non_index]
                    toxic_z2, non_z2 = None, None
            
            if 'mse' in self.ablation:
                mse = self.mse_loss(toxic_z1, toxic_z2, non_z1, non_z2)            
                mse_loss = mse
